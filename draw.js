...
function renderWorld() {
	if(info != null) {
		...
		ctx.fillStyle = "white";
		ctx.fillText(info.episode, render.canvas.wdith - ctx.measureText(info.episode).width - 20, 30);

		ctx.font = "40px Verdana";
		ctx.fillStyle = "white";
		ctx.fillText(info.score, render.canvas.wdith - ctx.measureText(info.score).width - 20, 70);
		...
		/*for(var i=0; i<N_SENSORS; i++) {
			...
			var mag = -(info.observation[i+2] * SCREEN_RESOLUTION - (SENSOR_RESOLUTION+1));
			ctx.beginPath();
			ctx.moveTo(player.position.x, player.position.y);
			ctx.lineTo(player.position.x + Math.cos(th)*mag*SENSOR_RANGE/SENSOR_RESOLUTION,
				player.position.y - Math.sin(th)*mag_SENSOR_RANGE/SENSOR_RESOLUTION);
			ctx.stroker();
		}*/
