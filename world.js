...
var render = Render.create({
	canvas: myCanvas,
	//element: document.body, //have to see what happens
	engine: engine,
	options: {
		wireframes: false,
		background: '#ccc'
...
function spawnBall() {
	...
	World.push({body: ball, velX: Common.random(3, 10)});
...
function lethal(body) {
	return ballBodes.indexOf(body) != -1/* || body == wallL || body = wallR*/;
}
...
function getSensors() {
	...
	for(let i=0; i<param.numSensors; i++) {
		const th = Math.PI - Math.PI / (params.numSensors-1) * i;
		var hit = 0;

		for(let k=1; k<=res; k++) {
			let results = Matter.Query.ray(ballBodes, player.position,
